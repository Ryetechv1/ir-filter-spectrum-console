...
- `Flashlight Studio` adds four managed rear-torch modes: `Hold Torch` for persistent on/off use, `Lock Rear Torch` for rear-stream lock/reapply behavior, `Start Strobe` with an 80ms-2000ms interval slider, and `Dimmer Pulse` with a 5%-100% duty-cycle brightness slider. Browser torch APIs generally expose on/off control, so the web app uses duty-cycle dimming when direct brightness is unavailable. The panel includes a local debug log for unsupported streams, failed constraints, strobe state changes, and dimmer-duty changes.
