...
- `Flashlight Studio` adds four managed rear-torch modes: `Hold Torch` for persistent on/off use, `Lock Rear Torch` for rear-stream lock/reapply behavior, `Start Strobe` with an 80ms-2000ms interval slider, and `Dimmer Pulse` with a 5%-100% duty-cycle brightness slider. The web app now probes direct float-like torch constraints (`torchLevel`, `brightness`, and `intensity`) before falling back to browser on/off torch plus duty-cycle dimming. The panel includes a local debug log and a direct-vs-duty-cycle capability readout for unsupported streams, failed constraints, strobe state changes, and dimmer-duty changes.
- `integrations/ios_torch_dimmer/TorchDimmerApp.swift` provides a native SwiftUI iPhone handoff for true smooth torch brightness from `0.1...1.0`. It uses AVFoundation `AVCaptureDevice.setTorchModeOn(level:)`, safely wraps configuration changes in `lockForConfiguration()` / `defer unlockForConfiguration()`, and should be tested on a physical iPhone because the iOS Simulator does not expose real torch hardware.
...
- The studio estimates pseudo-depth, contour bands, fine low-visibility point-cloud dots, field curvature, grid cells, TIN triangle facets, and barely visible mesh overlays from visible luminance/edge/noise gradients in the current camera/compositor canvas.
- The 42-control spatial stack now includes a 1000% low-light sensitivity amplifier for dim scenes, plus micro contrast, near/far field bias, occlusion, contour density/threshold, surface-normal response, specular/shadow/highlight sensitivity, color depth, noise rejection, object cohesion, motion trace, subpixel scan, thermal-lock weighting, live point-cloud strength, TIN facet opacity, TIN wire strength, cell size, cell-depth mapping, surface mapping, noise mapping, grid warp, point-height lift, and facet smoothing.
- Point Cloud and TIN are live integrated features for both front and rear device cameras. Each sampled cell reads visible pixels from the current rendered camera/upload canvas, estimates pseudo-depth/noise/surface-normal values, draws finer point samples, and fills/strokes TIN facets at reduced opacity so the visible preview, HUD, snapshots, video recording canvas, and overlay compositor share the same geometry layer without visibly covering the scene.
- Spatial Power is detail-weighted instead of center-radial, so raising it should not create a central light point. Parallax Drift now uses a single uniform translated layer across the whole preview rather than per-row or per-column fabric-like warping.
